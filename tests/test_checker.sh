...
for case in "${cases[@]}"; do
    ...
    for ARG in "${inputs[@]}"; do
        ...
        result_linux=$(./push_swap $ARG 2>/dev/null | ./checker_linux $ARG 2>/dev/null)
        ops=$(./push_swap $ARG 2>/dev/null | wc -l)
        
        if [ "$result" == "$result_linux" ]; then
			echo -e "${GREEN}Result: [$result] [$result_linux]${RESET} $ops [$ARG]"
        elif [ "$result" != "$result_linux" ]; then
            echo -e "${RED}Result: [$result] [$result_linux]${RESET} $ops [$ARG]"
		else 
			echo -e "Else: $result $ops [$ARG]"
        fi
    done
done
